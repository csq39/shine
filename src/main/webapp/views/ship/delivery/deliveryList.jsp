...
<script type="text/javascript">
	...
	function fncSendOtmsList() {
		...
		var carrier=new Array();
		 ...
		 $.ajax({
		     type: "post",
		     url: "${ctx}/ship/delivery/sendDeliveryList",
		     data: {"billcode":billcode.join(),"carrier":carrier.join(),"type":type.join()} ,
		     async:false,
		     success: function(){
	...
	function fncSendOtms(billcode,type,id) {
		var arr = table.rows().data();
		var carrier;
		for(var i = 0; i <arr.length;i++){
			if(arr[i].id == id){
		...
		$.ajax({
		     type: "post",
		     url: "${ctx}/ship/delivery/sendDelivery",
		     data: {"billcode":billcode,"carrier":carrier,"type":type} ,
		     async:false,
		     success: function(){
				rayDialog("操作完成",function(){
					table.ajax.reload();
...
<div class="page-container manage">
	...
	<form  id="form" method="post" action="/ship/delivery/queryDeliveryList">
		<span class="r ">
			状态：<rt:select2 id="outputStatus" dataQuery="yes"  headerKey="-1" className="w140 radius size-S" headerValue="全部"  comCode="cargotype" ></rt:select2>
			配方颗粒：<rt:select2 id="materialType" dataQuery="yes"  headerKey="-1" className="w140 radius size-S" headerValue="全部"  comCode="materialType" ></rt:select2>
			发运类型：<rt:select2 id="carrier" dataQuery="yes"  headerKey="-1" className="w140 radius size-S" headerValue="全部"  comCode="carrier" ></rt:select2>
			交货单:<input type="text" id="billcode" data-query="yes" class="input-text w140 radius size-S">
		...
		<table class="table table-border table-bordered table-bg table-hover table-sort table-responsive" id="datatable-form"  width="100%" style="wiite-spac:nowrap">
			<thead>
				<tr>
					...
					<th data-column="billcode" >交货单</th>
					<th data-column="outputStatus" >状态</th>
					<th data-column="deliveryBillcode" >销售单</th>
					<th data-column="materialType" >配方颗粒</th>
					...
					<th data-column="custid" >月结账号</th>
					<th data-column="op" data-method="[
								{targets:-2,params:'carrier',
								htmlAppend:'<select id=\'carrier\' name=\'carrier\' class=\'w50 radius size-S\' ><option value=\'OTMS\'>OTMS</option> <option  value=\'SF\'>顺丰</option> <option value=\'EMS\'>EMS</option></select>'}]">承运商</th>
					<th data-column="op"  data-method="[{targets:-1,title:'发送',fncName:'fncSendOtms',params:'billcode,type,id',icon:'detail'}]">操作</th>
				</tr>
			</thead>
			<tbody></tbody>
		</table>
