package io.rain.web.support.presentation.tag;
...
import io.rain.context.bean.SystemMenu;
import io.rain.core.application.SpringContextHolder;
import io.rain.core.collection.RData;
import io.rain.web.support.communal.dao.SecurityDao;
			...
			if(menu.getParentId().equals(parantId)){
				if(menu.getParentId().equals(SystemMenu.getRootId())) {//一级菜单
					...
					sb.append("<li><a data-href=\""+getMenuUrl(menu.getHref())+"\" data-menuId=\""+menu.getId()+"\" style=\"font-weight: inherit;\" data-title=\""+menu.getName()
						+"\" href=\"javascript:void(0)\">"+menu.getName()+"</a></li>\r\n");
					if(((SystemMenu)rdata.get(menu.getParentId())).getParentId().equals(SystemMenu.getRootId())) {
						sb.append( "</ul>");
					}
				}else{//二级有下级菜单
					rdata.set(menu.getId(), menu);
					sb.append("<dl>"
							+ "<dt>"+getMenuName(menu.getName(),menu.getIcon())+"<i class=\"Hui-iconfont menu_dropdown-arrow\">&#xe6d5;</i></dt>"
							+ "<dd><ul>"
							+ getMenuList(menuList,menu.getId(),rdata)
							+"</ul></dd>"
							+ "</dl>");
